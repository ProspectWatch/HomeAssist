...
export async function getRecipe(id: string): Promise<RecipeDetail | null> {
  try {
    ...
    const { data, error } = await supabase
      .from("recipes")
      .select("id, name, time_minutes, servings, recipe_ingredients(id, name, qty, sort_order, retailer:retailers(name))")
      .eq("id", id)
      .maybeSingle();
    ...
    type Row = {
      ...
      time_minutes: number | null;
      servings: string | null;
      recipe_ingredients: { id: string; name: string; qty: string | null; sort_order: number; retailer: { name: string } | null }[];
    };
    ...
    return {
      ...
      ingredients: [...r.recipe_ingredients]
        ...
        .map((i) => ({ id: i.id, name: i.name, qty: i.qty, retailer: i.retailer })),
    };
  } catch {
    return null;
  }
